...
//    c. Implement the code to count the leaf nodes int leafCount(){}
    public int leafCount(Node<E> node) {
        // node is null
    ...
    public int countInRange(Node<E> node, E bottom, E top) {
        if(node == null) return 0;
        int count = 0;
        if(node.data.compareTo(bottom) >= 0 && node.data.compareTo(top) <= 0) {
            count =  1;
        }
        return count + countInRange(node.left, bottom, top) + countInRange(node.right, bottom, top);
    }

    public int countInRange(E bottom, E top) {
        return countInRange(root, bottom, top);
    }

    // count how many node in the range from a to b
    // 50 <= x <= 80
    // count how many x
